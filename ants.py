...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        is_queen = isinstance(insect, QueenAnt)
        if not is_queen or (is_queen and insect.is_impostor):
            if insect.is_ant:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        """Take the 'other' ant and returns True when:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive:
            if self.in_range(count_range) and len(current_place.bees) > 0:
    ...
    def in_range(self, pos):
        """Return true, if the position to be attacked is within range
        or if there is no constraint on the range; false otherwise.
        """
        within_min_range = (self.min_range == None or pos >= self.min_range)
        within_max_range = (self.max_range == None or pos <= self.max_range)
        return within_min_range and within_max_range
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if current_place.ant == None:
            for bee in current_place.bees[:]: # Loop over a slice copy of the entire list.
                bee.reduce_armor(burn)
        # END Problem 4
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt has higher armor, but cannot attack."""
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        for bee in self.place.bees[:]: # Loop over a slice copy of the entire list.
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower does not lose its armor when placed in Water."""
...
class BodyguardAnt(Ant):
    ...
    def reduce_armor(self, amount):
        """Reduce armor by AMOUNT, and remove the insect from its place if it
        has no armor remaining. If it is a BodyguardAnt, wich has a contained
        Ant, leave the contained Ant in place.
        """
        self.armor -= amount
        if self.armor <= 0:
            guarded_ant = self.ant
            current_place = self.place
            self.place.remove_insect(self)
            current_place.ant = guarded_ant
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_impostor:
            ...
            current_place = self.place
            if self is not current_place.ant and self not in QueenAnt.doubled_ants:
                self.double_damage(current_place.ant)
            current_place = current_place.exit
            while current_place is not None:
                ant = current_place.ant
                if ant is not None:
                    if ant not in QueenAnt.doubled_ants:
                        self.double_damage(ant)
                    if (ant.container and ant.ant is not None) and ant.ant not in QueenAnt.doubled_ants:
                        self.double_damage(ant.ant)
                current_place = current_place.exit
        # END Problem 13

    def double_damage(self, ant):
        """ This method doubles the damage of the ant, and adds it to the list
        of ants, whose damage has been doubled.
        """
        ant.damage *= 2
        QueenAnt.doubled_ants.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and not is_impostor:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    DURATION is updated via the counter variable AFFECTED_ROUNDS in BEE.
    A BEE knows if it is AFFECTED."""
    # BEGIN Problem EC
    counter = 0
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            apply_effect(make_slow, target, self.duration)
...
class StunThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            apply_effect(make_stun, target, self.duration)
